...
import com.paori.mobileServer.dto.ResponseMemberDTO;
...
public class MemberController {
	...
	public ResponseEntity<?> loginMember(MemberDTO dto) {
		...
		try {
			...
			if(result == null) {
				response = ResponseMemberDTO.builder().error("없는 이메일이나 잘못된 비밀번호입니다.").build();
			} else {
	...
	public ResponseEntity<?> getMember(MemberDTO dto) {
		...
		try {
			...
			if(result == null) {
				response = ResponseMemberDTO.builder().error("없는 이메일입니다.").build();
			} else {
				response = ResponseMemberDTO.builder()
						.email(result.getEmail())
						.name(result.getName())
