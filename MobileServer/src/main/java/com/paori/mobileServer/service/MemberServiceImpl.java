...
public class MemberServiceImpl implements MemberService {
	...
	// application.yml 파일에 작성한 속성 가져오기
	...
	public String registerMember(MemberDTO dto) {
		...
		if (uploadFile.isEmpty() == false) {
			...
			// 이미지 경로를 DTO에 설정
			dto.setImageurl(realUploadPath.replace('\\', '/') + "/" + uuid + fileName);
		}
	...
	public MemberDTO loginMember(MemberDTO memberDTO) {
		...
		if (optional.isPresent()) {
			...
			if (BCrypt.checkpw(memberDTO.getPassword(), member.getPassword())) {
				...
				Member updateMember = Member.builder().email(member.getEmail()).password(member.getPassword())
						.name(member.getName()).tel(member.getTel()).ismanager(member.getIsmanager())
						.imageurl(member.getImageurl()).lastlogindate(now).build();
